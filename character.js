...
class Character {
  ...
  constructor(x, y) {
    ...
    this.maxSpeed = 4;
    // force maximale appliquée au personnage
    this.maxForce = 0.2;
    this.color = "white";
    ...
    // rayon du personnage pour l'évitement
  ...
  // seek et avoid
  applyBehaviors(target, obstacles, characters) {

    let seekForce = this.arrive(target);
    let avoidForce = this.avoidCorrige(obstacles);
    let separateForce = this.separate(characters);
  ...
  avoid(obstacles, characters) {
    ...
    // calcul d'un vecteur ahead devant le personnage
    ...
    let vehiculeLePlusProche = this.getVehiculeLePlusProche(characters)
  ...
  // Exerce une force renvoyant vers le centre du canvas si le personnage s'approche
  // des bords du canvas
  boundaries() {
    const d = 25;
    ...
    // si le personnage est trop à gauche ou trop à droite
    if (this.pos.x < d) {
  ...
  getVehiculeLePlusProche(characters) {
  ...
  getClosestObstacle(pos, obstacles) {
    // on parcourt les obstacles et on renvoie celui qui est le plus près du personnage
  ...
  /* Poursuite d'un point devant la target !
     cette methode renvoie la force à appliquer au personnage
  */
  pursue(character) {
    let target = character.pos.copy();
    let prediction = character.vel.copy();
    prediction.mult(10);
  ...
  evade(character) {
    let pursuit = this.pursue(character);
    pursuit.mult(-1);
    return pursuit;
  }
  ...
  // applyForce est une méthode qui permet d'appliquer une force au personnage
  ...
  // On dessine le personnage, le chemin etc.
  show() {
    // dessin du chemin
    this.drawPath();
    // dessin du vehicule
  ...
  drawCharacter() {
    ...
    // et on le tourne. heading() renvoie l'angle du vecteur vitesse (c'est l'angle du personnage)
    rotate(this.vel.heading());

    // Dessin d'un personnage sous la forme d'un triangle. Comme s'il était droit, avec le 0, 0 en haut à gauche
    ...
    // Cercle pour évitement entre personnages et obstacles
    if (Character.debug) {
      stroke(255);
      noFill();
  ...
  drawVector(pos, v, color) {
    ...
    // Il part du centre du personnage et va dans la direction du vecteur vitesse
    strokeWeight(3);
    stroke(color);
  ...
  showHealthBar() {
    ...
    fill(255, 0, 0);
    rectMode(CORNER);
    let barWidth = this.r_pourDessin * 2;
    // Positionne la barre au-dessus du personnage
    rect(this.pos.x - this.r_pourDessin, this.pos.y - this.r_pourDessin - 10, barWidth, 5);

    // Barre de vie (verte)
    fill(0, 255, 0);
    let currentHealthWidth = barWidth * (this.health / this.maxHealth);
    rect(this.pos.x - this.r_pourDessin, this.pos.y - this.r_pourDessin - 10, currentHealthWidth, 5);
    pop();
    }
}
